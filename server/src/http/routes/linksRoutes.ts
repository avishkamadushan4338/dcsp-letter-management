  ...
  Effect.gen(function* () {
    ...
    yield* HttpRouter.add("POST", "/api/links/:token/action",
      Effect.gen(function* () {
        ...
        const body = (yield* HttpServerRequest.schemaBodyJson(Schema.Unknown)) as ActionBody;
        if (!body.notes) {
    ...
    yield* HttpRouter.add("POST", "/api/links/:token/reassign",
      Effect.gen(function* () {
        ...
        const body = (yield* HttpServerRequest.schemaBodyJson(Schema.Unknown)) as ReassignBody;
        if (!body.officerId) {
